...
import { BelfitPlus, chargerProgramme, programme, programmeCharge, progOuvert } from './BelfitPlus.jsx';
    ...
    <div class="pg-coach">
      ...
      {!pr && (
        <div class="cp-carte cp-carte--or">
          ...
          <button class="cp-bt cp-bt--or" onClick={() => setAchat('plan')}>Demander mon plan</button>
        </div>
      )}
      {pr && (
        <div class={'cp-carte' + (rappel ? ' cp-carte--or' : '')}>
          ...
          <button class={'cp-bt ' + (rappel ? 'cp-bt--or' : 'cp-bt--gris')} onClick={() => setAchat('maj')}>Mettre à jour mon plan</button>
        </div>
      )}
